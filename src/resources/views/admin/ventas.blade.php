        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                <div class="bg-white border border-[#C5BBB0]/30 p-5">
                    ...
                    <p class="font-serif text-3xl text-[#1A1612] leading-none mb-1">{{ $kpis['reservas_hoy'] ?? 38 }}</p>
                ...
                <div class="bg-white border border-[#C5BBB0]/30 p-5">
                    ...
                    <p class="font-serif text-3xl text-[#1A1612] leading-none mb-1">${{ $kpis['ingresos_mes'] ?? '48k' }}</p>
                ...
                <div class="bg-white border border-[#C5BBB0]/30 p-5">
                    ...
                    <p class="font-serif text-3xl text-[#1A1612] leading-none mb-1">{{ $kpis['ocupacion'] ?? '74%' }}</p>
                ...
                <div class="bg-white border border-[#C5BBB0]/30 p-5">
                    ...
                    <p class="font-serif text-3xl text-[#1A1612] leading-none mb-1">{{ $kpis['cancelaciones'] ?? 5 }}</p>
            ...
            <div x-show="tab === 'reservas'">
                ...
                <div class="flex flex-col sm:flex-row gap-3 mb-6">
                    ...
                    <select class="border border-[#C5BBB0]/50 bg-white px-4 py-2 text-sm text-[#1A1612] focus:outline-none focus:border-[#C9A84C] rounded-none">
                        ...
                        @foreach($exposiciones ?? ['Arte Clásico','Historia Natural','Arte Contemporáneo','Civilizaciones Antiguas','Ciencia y Tecnología','Geografía y Cultura'] as $expo)
                            ...
                            <tbody>
                                ...
                                <tr class="border-b border-[#C5BBB0]/10 hover:bg-[#C9A84C]/[0.03] transition-colors">
                                    ...
                                    <td class="px-5 py-3">
                                        ...
                                        <button @click="cancelModal = true; cancelCodigo = '{{ $reserva->codigo }}'"
                                ...
                                {{-- Datos de muestra --}}
                                @foreach([
                                    ['MUS-4872','Ana García','Arte Clásico','21 Abr','14:00','240','confirmed'],
                                    ['MUS-4871','Carlos Mendoza','Historia Natural','21 Abr','11:00','300','confirmed'],
                                    ['MUS-4870','Luisa Hernández','Civilizaciones Antiguas','22 Abr','10:00','180','pending'],
                                    ['MUS-4869','Pedro Ramírez','Arte Contemporáneo','23 Abr','13:00','100','confirmed'],
                                    ['MUS-4868','Sofía Torres','Arte Clásico','24 Abr','15:00','120','cancelled'],
                                ] as [$codigo, $visitante, $expo, $fecha, $hora, $total, $estado])
                                <tr class="border-b border-[#C5BBB0]/10 hover:bg-[#C9A84C]/[0.03] transition-colors">
                                    <td class="px-5 py-3 text-[12px] font-mono text-[#8C8070] bg-[#F5F1EC] tracking-wider">{{ $codigo }}</td>
                                    <td class="px-5 py-3 text-[13px] text-[#1A1612]">{{ $visitante }}</td>
                                    <td class="px-5 py-3 text-[13px] text-[#8C8070]">{{ $expo }}</td>
                                    <td class="px-5 py-3 text-[13px] text-[#8C8070]">{{ $fecha }}</td>
                                    <td class="px-5 py-3 text-[13px] text-[#8C8070]">{{ $hora }}</td>
                                    <td class="px-5 py-3 text-[13px] text-[#1A1612] font-medium">${{ $total }}</td>
                                    <td class="px-5 py-3">
                                        @if($estado === 'confirmed')
                                            <span class="inline-block px-2 py-0.5 text-[10px] uppercase tracking-[0.1em] bg-[#E8F4EE] text-[#1A6B3C]">Confirmada</span>
                                    ...
                                    <td class="px-5 py-3">
                                        @if($estado !== 'cancelled')
                                        <button @click="cancelModal = true; cancelCodigo = '{{ $codigo }}'"
                                            class="border border-[#C5BBB0]/40 px-3 py-1 text-[10px] uppercase tracking-[0.1em] text-[#8B2020] hover:border-[#8B2020] hover:bg-[#8B2020] hover:text-white transition-colors">
                                            Cancelar
                                        </button>
                                        @else
                    ...
                    <div class="bg-white border border-[#C5BBB0]/30 overflow-hidden">
                        ...
                        <table class="w-full border-collapse">
                            ...
                            <tbody>
                                ...
                                @empty
                                @foreach([
                                    ['Arte Clásico', 142, '$17,040'],
                                    ['Historia Natural', 98, '$14,700'],
                                    ['Civilizaciones', 76, '$13,680'],
                                    ['Arte Contemporáneo', 63, '$6,300'],
                                    ['Ciencia y Tec.', 51, '$6,630'],
                                ] as [$nombre, $reservas, $ingresos])
                                <tr class="border-b border-[#C5BBB0]/10 hover:bg-[#C9A84C]/[0.03]">
                                    <td class="px-5 py-3 text-[13px] text-[#1A1612]">{{ $nombre }}</td>
                                    <td class="px-5 py-3 text-[13px] text-[#8C8070]">{{ $reservas }}</td>
                                    <td class="px-5 py-3 text-[13px] font-medium text-[#1A1612]">{{ $ingresos }}</td>
                                </tr>
                                @endforeach
                                @endforelse
                            </tbody>
                    ...
                    <div class="bg-white border border-[#C5BBB0]/30 overflow-hidden">
                        ...
                        <table class="w-full border-collapse">
                            ...
                            <tbody>
                                ...
                                @empty
                                @foreach([
                                    ['11:00', 94, 'Casi lleno', 'bg-[#FAEAEA] text-[#8B2020]'],
                                    ['12:00', 100, 'Lleno', 'bg-[#FAEAEA] text-[#8B2020]'],
                                    ['14:00', 87, 'Alto', 'bg-[#FDF3DC] text-[#7A5200]'],
                                    ['10:00', 62, 'Disponible', 'bg-[#E8F4EE] text-[#1A6B3C]'],
                                    ['17:00', 45, 'Disponible', 'bg-[#E8F4EE] text-[#1A6B3C]'],
                                ] as [$hora, $ocup, $label, $badge])
                                <tr class="border-b border-[#C5BBB0]/10 hover:bg-[#C9A84C]/[0.03]">
                                    <td class="px-5 py-3 font-serif text-base text-[#1A1612]">{{ $hora }} h</td>
                                    <td class="px-5 py-3 text-[13px] text-[#8C8070]">{{ $ocup }}%</td>
                                    <td class="px-5 py-3"><span class="inline-block px-2 py-0.5 text-[10px] uppercase tracking-[0.1em] {{ $badge }}">{{ $label }}</span></td>
                                </tr>
                                @endforeach
                                @endforelse
                            </tbody>
